#!/bin/bash
set -e
# because we must manage the openshift-hack e2e go mod and also the go-controller go mod, we must ensure they are
...
GO_MOD_OCP_HACK="$REPO_ROOT/openshift-hack/e2e/go.mod"

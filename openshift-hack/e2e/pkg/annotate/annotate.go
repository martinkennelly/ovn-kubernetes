...
type labelAppender struct {
	...
	// key is label which maps to a list of labels defined in openshift-hack/e2e/annotate/rules.go LabelToLabelMaps map
...
func (r *labelAppender) generate(name string, node types.TestSpec) {
	...
	if !r.excludedTestsFilter.MatchString(newName) && !strings.Contains(newName, "[Suite:") {
		...
		isSlow := strings.Contains(newName, "[Slow]")
		switch {
		case isSlow:
			newLabels += "[openshift/ovn-kubernetes/slow]"
		case isSerial:
			newLabels += "[openshift/ovn-kubernetes/serial]"
		default:
			newLabels += "[openshift/ovn-kubernetes/parallel]"
		}
	}

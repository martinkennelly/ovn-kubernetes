...
# Adds drop flows for GARPs on CDN patch port of specified bridge. If the bridge doesn't
# exist, do nothing and return
add_garp_drop_flow() {
    local bridge="$1"
    local cookie="0x0305"
    local priority="499"
    # if bridge exists, we expect to add at least one flow to a patch port ending in to-br-int.
...
ovnkube-controller-with-node() {
  trap 'kill $(jobs -p) ; rm -f /etc/cni/net.d/10-ovn-kubernetes.conf ; echo EXITINGshellscript####; exit 0' TERM
  check_ovn_daemonset_version "1.0.0"
